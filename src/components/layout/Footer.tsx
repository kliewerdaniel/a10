...
function FooterLink({ href, label, external }: { href: string; label: string; external?: boolean }) {
  if (external) {
    return (
      <a href={href} target="_blank" rel="noopener noreferrer" className="text-sm text-ink-3 hover:text-ink transition-colors font-bold">
        {label} ↗
      </a>
    );
  }
  return (
    <Link href={href} className="text-sm text-ink-3 hover:text-ink transition-colors font-bold">
      {label}
    </Link>
  );
}
    ...
    <footer className="border-t-4 border-ink bg-surface relative">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 relative">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-10">
          ...
          <div>
            <h3 className="font-display text-ink mb-5 text-xs">Research</h3>
          ...
          <div>
            <h3 className="font-display text-ink mb-5 text-xs">Projects</h3>
            <ul className="space-y-3">
              {projectLinks.map((l) => (
                <li key={l.href}><FooterLink href={l.href} label={l.label} external={l.external} /></li>
              ))}
          ...
          <div>
            <h3 className="font-display text-ink mb-5 text-xs">About</h3>
            <ul className="space-y-3">
              {aboutLinks.map((l) => (
                <li key={l.href}><FooterLink href={l.href} label={l.label} external={l.external} /></li>
              ))}
